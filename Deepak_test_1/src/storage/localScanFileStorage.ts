...
async function migrateFromLegacyTxt(base: string): Promise<RackScanRowPersisted[]> {
  try {
    ...
    const txts = entries.filter((f) => f.startsWith('rack_scan_') && f.endsWith('.txt'));
...
export async function loadSessionRows(): Promise<RackScanRowPersisted[]> {
  ...
  if (!jsonInfo.exists) {
    const migrated = await migrateFromLegacyTxt(base);
    if (migrated.length) await writeSessionJson(base, migrated);
    return migrated;
  }
...
export async function getSessionUri() {
  ...
  const body =
    [...rows]
      ...
      .join('\n') + '\n';
  const exportPath = base + EXPORT_TXT;
  await FileSystem.writeAsStringAsync(exportPath, body, { encoding: FileSystem.EncodingType.UTF8 });
  sessionUri = exportPath;
  return exportPath;
}
...
export async function updateRackScanRow(
  id: string,
  patch: Partial<Pick<RackScanRow, 'part_number' | 'sap_part_number'>>
): Promise<void> {
  const base = FileSystem.documentDirectory;
  if (!base) throw new Error('documentDirectory unavailable');
...
export async function deleteRackScanRow(id: string): Promise<void> {
  const base = FileSystem.documentDirectory;
  if (!base) throw new Error('documentDirectory unavailable');

  const rows = (await loadSessionRows()).filter((r) => r.id !== id);
  await writeSessionJson(base, rows);
}
...
export async function clearLocalScanFile() {
  ...
  if (base) {
    ...
    try {
      const entries = await FileSystem.readDirectoryAsync(base);
      const legacyTxts = entries.filter((f) => f.startsWith('rack_scan_') && f.endsWith('.txt'));
