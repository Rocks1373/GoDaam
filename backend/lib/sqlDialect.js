...
/** date(expr) → (expr)::date (SQLite date() helper) */
function replaceDateCalls(sql) {
  return replaceFunctionCalls(sql, 'date', 'date');
}
...
function translateSqlForPostgres(sql, params) {
  ...
  // Postgres: delivery_notes.dn_date is TEXT; COALESCE(timestamp, text, timestamp) errors (42804).
  // SQLite allows mixed types in COALESCE for date(); keep PG-only rewrite here.
  s = s.replace(
    /\(COALESCE\s*\(\s*delivered_at\s*,\s*dn_date\s*,\s*created_at\s*\)\s*\)::date/gi,
    '(COALESCE((delivered_at)::date, (NULLIF(TRIM(COALESCE(dn_date::text, \'\')), \'\'))::date, (created_at)::date))'
  );
